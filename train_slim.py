'''
Training of base network before fine-tuning for network slimming (inclusion of L1 sparsity for scale in BN)
source: https://github.com/Eric-mingjie/network-slimming/blob/master/main.py
'''
...
if __name__ == '__main__':
    ...
    if(args.dataset == "CIFAR10"):
        ...
        test_loader = torch.utils.data.DataLoader(
            torchvision.datasets.MNIST('/DATA/MNIST', train=False, transform=transforms.Compose([
                                transforms.ToTensor(),
                                transforms.Normalize((0.1307,), (0.3081,))
                            ])),
    ...
    if args.model == "vgg":
        model = vgg11(ratio=ratio, efficient_scale=False, num_classes=num_classes, prune_fname=args.prune_fname, descent_idx=args.descent_idx, pruned_filters=pruned_filters)
    elif args.model == "resnet18":
        model = PreActResNet18(ratio=ratio, efficient_scale=False, num_classes=num_classes, dataset=args.dataset, prune_fname=args.prune_fname, descent_idx=args.descent_idx, pruned_filters=pruned_filters, search=True)
    elif args.model == "mobilenetv2":
        model = MobileNetV2(ratio=ratio, efficient_scale=False, num_classes=num_classes, dataset=args.dataset, prune_fname=args.prune_fname, descent_idx=args.descent_idx, pruned_filters=pruned_filters, search=True)
    else:
    ...
    #     scheduler = StepLR(optimizer, int(2.4 * len(train_loader)), 0.97)
    ###############
    ## Training  ##
    ###############
    best_test_acc = 0
